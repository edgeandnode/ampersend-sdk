...
Two artifacts in this repo have separate spec + conformance files in `docs/spec/`. When editing either artifact, also
review and update its spec and regenerate its conformance:

- `skills/ampersend/SKILL.md` → `docs/spec/ampersend-skill/SPEC.md` + `CONFORMANCE.md`
- `docs/getting-started.md` → `docs/spec/getting-started/SPEC.md` + `CONFORMANCE.md`

The conformance file is hand-regenerated by reading the spec and the artifact and recording one PASS/FAIL line per rule.
There's no CI check today, so this is the only thing keeping artifact and spec from drifting.
