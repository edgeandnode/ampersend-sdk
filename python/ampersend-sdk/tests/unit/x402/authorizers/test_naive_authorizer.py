"""Unit tests for NaiveX402Authorizer."""

from unittest.mock import MagicMock

import pytest
from ampersend_sdk.x402 import X402Wallet
from ampersend_sdk.x402.authorizers import (
    NaiveX402Authorizer,
)


@pytest.mark.asyncio
class TestNaiveAuthorizer:
    """Test NaiveAuthorizer."""
    ...
    async def test_authorize_payment(self) -> None:
        ...
        authorizer = NaiveX402Authorizer(wallet=mock_wallet)
        ...
        result = await authorizer.authorize(mock_payment_required)
        assert result is not None

    async def test_onStatus_is_noop(self) -> None:
        """Test that onStatus does nothing for naive authorizer."""
        mock_wallet = MagicMock(spec=X402Wallet)
        mock_wallet.create_payment.return_value = MagicMock(name="PaymentPayload")
