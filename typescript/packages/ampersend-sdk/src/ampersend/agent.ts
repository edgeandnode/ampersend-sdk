...
export const AgentSelfSpendConfigDTO = Schema.extend(
  AgentSpendConfigDTO,
  Schema.Struct({
    daily_remaining: Schema.NullOr(Schema.BigInt),
    monthly_remaining: Schema.NullOr(Schema.BigInt),
  }),
)
...
/** Incoming top-up from the owner's main account. */
export const AgentTopupTransactionDTO = Schema.Struct({
  id: ID,
  main_account_address: Address,
  tx_hash: TxHash,
  chain_caip2id: Caip2ID,
  created_at: ConvertedTimestamp,
  updated_at: ConvertedTimestamp,
})
export type AgentTopupTransactionDTO = typeof AgentTopupTransactionDTO.Type
